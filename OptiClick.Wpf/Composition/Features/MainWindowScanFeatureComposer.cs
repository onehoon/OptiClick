...
internal static class MainWindowScanFeatureComposer
{
    public static MainScanResolvedDependencies Compose(ScanCompositionServices scan)
    {
        var scanFolderListController = new ScanFolderListController(
            scan.ScanFolderManifestStore,
            scan.ScanFileSystemProbe);
        return new MainScanResolvedDependencies
        {
            ...
            ScanVisibleGameResolver = new ScanVisibleGameResolver(),
            ScanFolderActionController = new ScanFolderActionController(
                scanFolderListController,
                scan.FolderPickerService),
            ScanResultCoordinatorFactory = new ScanResultCoordinatorFactory(),
            ScanOrchestratorFactory = new ScanOrchestratorFactory()
        };
    }
}
