...
public sealed class RtssProfileApplier : IInstallRtssProfileApplier
{
    ...
    public RtssProfileApplier()
        : this(RtssInstallPathResolver.Resolve, RestartRtssSilently)
    {
    }
    ...
    public InstallRtssProfileApplyResult Apply(InstallRtssProfileApplyRequest request)
    {
        ArgumentNullException.ThrowIfNull(request);
        var logs = new List<InstallRtssProfileApplyLogEntry>();
        try
        {
            ...
            {
                return new InstallRtssProfileApplyResult { Logs = logs };
            }

            var globalResult = ApplyGlobalSettings(globalPath);
            foreach (var item in globalResult.Items)
            {
                logs.Add(RtssProfileApplyLogEntryFactory.Info("config", FormatGlobalItemLog(item)));
            }

            var profileOutcome = ApplyGameProfileOverlay(
                request.RequiresRtssProfile,
                request.MatchExe,
                installPath,
                globalPath);
            ...
            logs.Add(RtssProfileApplyLogEntryFactory.Info("config", FormatProfileLog(profileOutcome, restart)));
        }
        catch (Exception ex)
        {
            logs.Add(RtssProfileApplyLogEntryFactory.Warning("config", $"[RTSS] Failed to apply RTSS settings: {ex.Message}"));
        }
        ...
        string globalPath)
    {
        try
        {
            ...
            var exeName = ResolveExeName(matchExe);
    ...
    private static string ResolveExeName(string matchExe)
    {
        var requiredFiles = MatchExePatternParser.ParseRequiredFiles(matchExe);
...
internal static class RtssProfileApplyLogEntryFactory
{
    public static InstallRtssProfileApplyLogEntry Info(string category, string message)
    {
        return new InstallRtssProfileApplyLogEntry
        {
            Level = "info",
            Category = category ?? "",
            Message = message ?? ""
    ...
    public static InstallRtssProfileApplyLogEntry Warning(string category, string message)
    {
        return new InstallRtssProfileApplyLogEntry
        {
            Level = "warning",
