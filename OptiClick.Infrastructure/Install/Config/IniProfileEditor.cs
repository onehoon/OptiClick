...
public sealed class IniProfileEditor
{
    ...
    public ConfigProfileApplySummary ApplyGameIni(string targetPath, ConfigApplyProfileRows profileRows)
    {
        var skipped = new List<ConfigProfileSkippedRow>();
        var groupedTargets = _targetCollector.CollectGameIniTargets(
            targetPath,
            (profileRows ?? ConfigApplyProfileRows.Empty).GameIniProfileRows,
            skipped);
        return ApplyGameIniTargets(groupedTargets, skipped);
    }
    ...
    public ConfigProfileApplySummary ApplyEngineIni(string targetPath, IReadOnlyDictionary<string, object?> gameData)
    {
        ...
        return ApplyEngineIniTargets(groupedTargets, skipped);
    }

    private static ConfigProfileApplySummary ApplyGameIniTargets(
        IReadOnlyDictionary<string, Dictionary<string, Dictionary<string, string>>> groupedTargets,
    ...
    public ConfigProfileApplySummary ApplyEngineIni(string targetPath, ConfigApplyProfileRows profileRows)
    {
        var skipped = new List<ConfigProfileSkippedRow>();
        var groupedTargets = _targetCollector.CollectEngineIniTargets(
            targetPath,
            (profileRows ?? ConfigApplyProfileRows.Empty).EngineIniProfileRows,
            skipped);
        return ApplyEngineIniTargets(groupedTargets, skipped);
    }

    private static ConfigProfileApplySummary ApplyEngineIniTargets(
        IReadOnlyDictionary<string, Dictionary<string, Dictionary<string, string>>> groupedTargets,
        List<ConfigProfileSkippedRow> skipped)
    {
