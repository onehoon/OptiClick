...
public sealed class UnrealIniProfileEditor
{
    ...
    public ConfigProfileApplySummary Apply(string targetPath, OptiClick.Core.Install.ConfigApplyProfileRows profileRows)
    {
        ...
        var targets = CollectTargets(
            targetPath,
            (profileRows ?? OptiClick.Core.Install.ConfigApplyProfileRows.Empty).GameUnrealIniProfileRows,
            skipped);
    ...
    private static ConfigProfileApplySummary ApplyTargets(
        IReadOnlyDictionary<string, List<UnrealIniRow>> targets,
        ...
        List<ConfigProfileSkippedRow> skipped)
    {
        ...
        foreach (var row in profileRows ?? [])
        {
            ...
            var section = ConfigApplyProfileRowReader.ReadSection(row);
            var key = ConfigApplyProfileRowReader.ReadKey(row);
            var valuePath = ConfigApplyProfileRowReader.ReadValuePath(row, "value_path");
            if (string.IsNullOrWhiteSpace(profilePath)
                || string.IsNullOrWhiteSpace(section)
                || string.IsNullOrWhiteSpace(key)
                || string.IsNullOrWhiteSpace(valuePath))
            ...
            list.Add(new UnrealIniRow
            {
                ...
                Value = IniProfileEditor.NormalizeProfileScalar(
                    ...
                    ConfigApplyProfileRowReader.ReadValueType(row))
            });
        }
